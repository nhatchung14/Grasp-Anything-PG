...
class DINO_GenerativeConvNet(GraspModel):
    ...
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, dropout=False, prob=0.0, is_aligned=0):
        ...
        self.postprocess_dino_conv = nn.ConvTranspose2d(1024, channel_size * 12, kernel_size=2, stride=2)
        self.postprocess_dino_bn = nn.BatchNorm2d(channel_size * 12)
    ...
    def forward(self, x_in, prompts):
        ...
        # route 2
        y = F.relu(self.bn1(self.conv1(x_in)))
        ...
        # combined route
        x = torch.cat([x, y], dim=1) # [bs, 512, hi, wi]
